...
void Print(unsigned char* data, int start, int size, unsigned char* dest=nullptr, bool print=true) {
...
void datParser(unsigned char* dat, unsigned int size,unsigned char * lastBlockMerkleRoot=nullptr) {
...
bool CompareDATFiles(int FileNo) {
...
static bool WriteBlockToDisk(const CBlock& block, FlatFilePos& pos, const CMessageHeader::MessageStartChars& messageStart)
{
    ...
    if(pos.nFile!=vfilePos) { //new datafile is created!
        ...
        CBlock block2 = block;
        compare_dat_files(pos.nFile-1);
        if (!CompareDATFiles(pos.nFile-1)) {
